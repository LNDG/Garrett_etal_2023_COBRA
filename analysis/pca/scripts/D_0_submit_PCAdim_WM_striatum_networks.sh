...
for ID in ${SUBJECTS}; do
	...
	echo "#PBS -N PCAdim_${modality}_network_masks_${ID}"									>> jobfile 			# job name
	echo "#PBS -l walltime=3:0:0" 											>> jobfile			# time until job is killed
	echo "#PBS -l mem=6gb"	 												>> jobfile			# books 4gb RAM for the job
	...
	echo "gunzip ${WD}/preproc_nback/preproc/C${ID}/C${ID}_nback_FEAT_detrend_filt_FIX_MNI2mm.nii.gz; fi" >> jobfile 	
	
	echo "cd ${WD}/dimensionality_nback/scripts/" 				>> jobfile

	echo "umask 0007"												 		>> jobfile 			# set permissions
    
	#echo "./run_D_1_${modality}PCAdim_WM_striatum_networks.sh /opt/matlab/R2016b/ ${ID}" 			>> jobfile
	echo "./run_D_2_${modality}PCAdim_Shirer_selected_networks.sh /opt/matlab/R2016b/ ${ID}" 			>> jobfile
