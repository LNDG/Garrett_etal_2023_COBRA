% this script calculates the dimensionality within the combined Tian Hippocampus/Yeo Networks Mask
% with the help of temporal PCA
...
BASEPATH='BASE';
PLSPATH=([BASEPATH, '/PLS/SD_2mm/']);
MASKPATH_hip = ([BASEPATH,'/COBRA_masks-selected/Tian_hippocampus/']);
...
SAVEPATH = ([BASEPATH,'/Dimensionality_nback/', modality, '_PCA/Hip_Tian_DMN_Yeo/']);
...
for i=1:length(ID)
    ...
    dst_run = 0;
      ...
      for cond = 1:numel(conditions)
       ...
       %% Dimensions
       data=0;
       data = cond_data{cond}; %only within network!
    ...
    PCAdim_networks(1, :)=Dimensions;
    disp([num2str(Dimensions)]);
    Var_explained_1stfactor(1, :)=explained_var;
    disp([num2str(explained_var)]);
    
        
    save([SAVEPATH, 'C', ID{i}, '_PCAdim_temp_HipDMN.mat'], 'PCAdim_networks', 'Var_explained_1stfactor');%note that EXPLAINED here is from typical, unrotated solution. 
    disp(['saved as ', SAVEPATH, 'C', ID{i}, '_PCAdim_temp_HipDMN.mat']);
